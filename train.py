...
def main():
    ...
    parser.add_argument('--data_path', default='/data/abhidip/MM_SRL_data/flickr/entity/flickr30k_entities-master/',
                        help='path to datasets')
    parser.add_argument('--tsv_path', default='/data/abhidip/MM_SRL_data/flickr/image_feat',
                        help='path to datasets')
    parser.add_argument('--data_json',default ='/data/abhidip/MM_SRL_data/flickr/flickrdata_VN_SRL_martha_BBOX2.json', help ="path to json file")
    ...
    parser.add_argument('--proposition_per_sen', default=3, type=float,
                        help='#of SRL prop per sentence to use.')
    ...
    parser.add_argument('--logger_name', default='/data/abhidip/MM_SRL_data/flickr/models/SRL_aware_ret/log',
                        help='Path to save Tensorboard log.')
    parser.add_argument('--model_name', default='/data/abhidip/MM_SRL_data/flickr/models/SRL_aware_ret/checkpoint',
    ...
    parser.add_argument('--attn_guide', default="normal",
                        help='normal|iou')
    ...
    img_data = []
    tsv_path1 = os.path.join(opt.tsv_path, "train.tsv")
    tsv_path2 = os.path.join(opt.tsv_path, "dev.tsv")
    tsv_path3 = os.path.join(opt.tsv_path, "test.tsv")
    ...
    raw_data = json.load(open(opt.data_json, "r"))

    # Load data loaders
    tr_dataset = CaptionDataset(raw_data, img_data , w_vocab, srl_vocab, bio_vocab, 'train', opt)
    vl_dataset = CaptionDataset(raw_data, img_data , w_vocab, srl_vocab,  bio_vocab, 'dev', opt)
    train_loader, val_loader = get_dataloader(tr_dataset, vl_dataset, opt)
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        # train for one epoch
        print("training at {}/{}".format(epoch, opt.num_epochs))
...
def validate(opt, val_loader, model):
    ...
    # caption retrieval
    (r1, r5, r10, medr, meanr) = i2t_any(img_embs, cap_embs, cap_lens, sims)
    logging.info("Image to text: %.1f, %.1f, %.1f, %.1f, %.1f" %
                 (r1, r5, r10, medr, meanr))
    # image retrieval
    (r1i, r5i, r10i, medri, meanr) = t2i_any(img_embs, cap_embs, cap_lens, sims)
